...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (inviteCode.trim()) {
        try { await db.functions.invoke("verifyInviteCode", { code: inviteCode.trim(), email }); } catch { /* invalid code — proceed with login */ }
      }
  ...
  const handleGoogle = () => {
    // Persist the signup intent so the post-auth role bootstrap can apply it after the OAuth redirect.
    try { sessionStorage.setItem("plivex:signupIntent", inviteCode.trim() || "independent"); } catch { /* best effort */ }
